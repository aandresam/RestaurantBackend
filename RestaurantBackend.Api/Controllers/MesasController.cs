...
public sealed class MesasController(IMesaService mesaService) : ControllerBase
{
    ...
    public async Task<IActionResult> List(CancellationToken cancellationToken)
        => Ok(await mesaService.ListAsync(cancellationToken).ConfigureAwait(false));

    [HttpGet("{nroMesa:int}")]
    public async Task<IActionResult> GetByNroMesa([FromRoute] int nroMesa, CancellationToken cancellationToken)
    {
        var result = await mesaService.GetByNroMesaAsync(nroMesa, cancellationToken).ConfigureAwait(false);
    ...
    {
        var result = await mesaService.CreateAsync(request, cancellationToken).ConfigureAwait(false);
    ...
    {
        var updated = await mesaService.UpdateAsync(nroMesa, request, cancellationToken).ConfigureAwait(false);
    ...
    {
        var deleted = await mesaService.DeleteAsync(nroMesa, cancellationToken).ConfigureAwait(false);
